...
public class MidiFile {
    ...
    private ArrayList<MidiTrack> tracksNew;
	...
	int iMeasureStartIndex;//measure��ʲô��ʼ����
	int iQuarterOne;
	
	ArrayList<ArrayList<Integer>> oneNoteMeasure = new ArrayList<ArrayList<Integer>>(2);
	ArrayList<Integer> oneMeasure1 = new ArrayList<Integer>();
	ArrayList<Integer> oneMeasure2 = new ArrayList<Integer>();
	ArrayList<ArrayList<Integer>> oneRestMeasure = new ArrayList<ArrayList<Integer>>(2);
	ArrayList<Integer> oneRestMeasure1 = new ArrayList<Integer>();
	ArrayList<Integer> oneRestMeasure2 = new ArrayList<Integer>();
	int MeasurelastStart;
	
	int iMidiStartTime = 0;
	boolean bFlagReadMidiStartTime = false;
	
	public boolean bFlagXmlRight = true;
	
	public int iTempoNew;
	public int tempouse;
	
	ArrayList<MidiEvent> mergeMidiEvents = null;

	// add end

	/*
	 * The Program Change event gives the instrument that should be used for a
	 * particular channel. The following table maps each instrument number (0
	 * thru 128) to an instrument name.
	 */
	public static String[] Instruments = { "Acoustic Grand Piano",
			...
			"Helicopter", "Applause", "Gunshot", "Percussion" };
    ...
    ArrayList<MidiEvent> mergeMidiEvents()
    {
    	if(mergeMidiEvents == null)
    	{
    		
    	}
    	return mergeMidiEvents;
    }
	...
	public static byte[] getByte(File file) throws Exception {
		...
		if (file != null) {
			...
			if (length > Integer.MAX_VALUE) // 当文件的长度超过了int的最大�??
			{
    ...
    {
    	bFlagReadMidiStartTime = false;
		oneNoteMeasure.add(oneMeasure1);
		oneNoteMeasure.add(oneMeasure2);
		
		oneRestMeasure.add(oneRestMeasure1);
		oneRestMeasure.add(oneRestMeasure2);
		
        String id;
        int len;

        tracks = new ArrayList<MidiTrack>();
        tracksNew = new ArrayList<MidiTrack>();
        trackPerChannel = false;
        
//        File filetest = new File("/mnt/sdcard/小汤8-16-圆舞�?.mid");//小汤8-16-圆舞�?
//        byte[] testData = new byte[100];
//        try {
//        	testData = getByte(filetest);
//		} catch (Exception e) {
//			// TODO Auto-generated catch block
//			e.printStackTrace();
//		}

        MidiFileReader file = new MidiFileReader(rawdata);//rawdata
        id = file.ReadAscii(4);
		...
		iQuarterOne = quarternote;

		allevents = new ArrayList<ArrayList<MidiEvent>>();
		for(int i = 0; i < num_tracks; i ++)
		{
			allevents.add(ReadTrack(file));
			 MidiTrack track = new MidiTrack(allevents.get(i),
			 i);
			if (track.getNotes().size() > 0) {
				tracks.add(track);
				mergeMidiEvents = allevents.get(allevents.size() - 1);
			}
		}
		
//		for (int tracknum = 0; tracknum < 2; tracknum++) {//num_tracks
//			
//			// MidiTrack track = new MidiTrack(allevents.get(tracknum),
//			// tracknum);
//			MidiTrack track = new MidiTrack(this, listNote.get(tracknum),
//					tracknum, getMeasure());
//			//if (track.getNotes().size() > 0) 
//			{
//				tracks.add(track);
...
//		}
    ...
    private ArrayList<MidiEvent> ReadTrack(MidiFileReader file) {
        ...
        int tracklen = file.ReadInt();		//read one  MTrk
        int trackend = tracklen + file.GetOffset();
        Log.e(TAG, "tracklen "+tracklen+" file.GetOffset "+file.GetOffset()+" trackend "+trackend);
        byte eventflag = 0;
        ...
        while (file.GetOffset() < trackend) {
            ...
            try {
               ...
               // Log.e(TAG, "startoffset "+startoffset+" deltatime "+deltatime+" starttime "+starttime+" peekevent "+peekevent);
            }
            ...
            if (peekevent < 0) {
                mevent.hasEventful = true;
                eventflag = file.ReadByte();
            }

/*            Log.e(TAG,  "offset " + startoffset + 
            ...
            if (eventflag >= EventNoteOn && eventflag < EventNoteOn + 16) {
            	if(!bFlagReadMidiStartTime)
            		iMidiStartTime = starttime;
    ...
    public ArrayList<MidiTrack> ChangeMidiNotes(MidiOptions options) {
        ...
        //Torres Mark 由于不是标准的midi，暂时屏蔽，不然虚拟键左右手有问�?
        //MidiFile.RoundStartTimes(newtracks, options.combineInterval, timesig);
        
        
		// MidiFile.RoundDurations(newtracks, time.getQuarter()); //Torres
		// delete
    ...
    public static ArrayList<MidiTrack> SplitTrack(MidiTrack track, int measurelen) {
		...
		for (MidiNote note : notes) {
			...
			// * (that start exactly at this start time), choose the closest
			// one.
			// * - If this note is more than an octave from the high/low notes
			// * (in this note's time duration), choose the closest one.
			// * - If the high and low notes (that start exactly at this
			// starttime)
			// * are more than an octave apart, choose the closest note.
			// * - If the high and low notes (that overlap this starttime)
			// * are more than an octave apart, choose the closest note.
			// * - Else, look at the previous high/low notes that were more than
			// an
			...
			//
			// FindHighLowNotes(notes, measurelen, startindex,
			// note.getStartTime(), note.getEndTime(), pair);
			// FindExactHighLowNotes(notes, startindex, note.getStartTime(),
			// pairExact);
			//
    ...
    public static ArrayList<MidiTrack> CombineToTwoTracks(ArrayList<MidiTrack> tracks, int measurelen)
    {
		...
		// ArrayList<MidiTrack> result = SplitTrack(single, measurelen);//Torres
		// delete
		ArrayList<MidiTrack> result = new ArrayList<MidiTrack>(2);
		if(tracks.size() > 1)
		{
			result.add(tracks.get(0));
			result.add(tracks.get(1));
		}
		else if(tracks.size() == 1)
			result.add(tracks.get(0));
    ...
    RoundStartTimes(ArrayList<MidiTrack> tracks, int millisec, TimeSignature time) {
		...
		// CheckStartTimes(tracks);//Torres delete
    ...
    public int EndTime() {
        ...
        return MeasurelastStart;//lastStart;
    }
