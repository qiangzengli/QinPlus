...
public class WheelView extends View {
     ...
     * 默认字体
     */
    private float normalFont = 40.0f;//px
    /**
     * 选中的时候字体
     */
    private float selectedFont = 60.0f;//px
    /**
     * 单元格高度
     */
    private float unitHeight = 100;//px
    /**
     * 显示多少个内容
     */
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        maskDarkColor =
                attribute.getColor(com.pl.whellview.R.styleable.WheelView_maskDarkColor, DEFAULT_MASK_DARK_COLOR);
        maskLightColor =
                attribute.getColor(com.pl.whellview.R.styleable.WheelView_maskLightColor, DEFAULT_MASK_LIGHT_COLOR);
        ...
        if (mastPaint == null) {
            ...
            linearGradientDown =
                    new LinearGradient(0, controlHeight - unitHeight, 0, controlHeight, maskLightColor,
                            maskDarkColor, Shader.TileMode.CLAMP);
        }
        ...
        moveHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                if (selected != -1) {
                    ...
                    synchronized (toShowItems) {
                        if (move > 0) {
                            for (ItemObject toShowItem : toShowItems) {
                                if (toShowItem != null && toShowItem.couldSelected()) {
                                    newMove = (int) toShowItem.moveToSelected();
    ...
    private class ItemObject {
        ...
        public void drawSelf(Canvas canvas, int containerWidth) {
            ...
            if (itemText.contains("-")) {
                // 绘制内容
                String[] strings = itemText.split("-");
                LogUtil.i(TAG, "strings[0]=" + strings[0] + "strings[1]=" + strings[1] + "strings.length=" + strings.length);
                if (!StringUtils.isEmpty(strings[0])) {
                    textPaint.setTextSize(normalFont);
                    canvas.drawText(strings[0],
                            ((float) x + (float) controlWidth / 2f - 10),
                            ((float) (y + move) + unitHeight / 2 - 5), textPaint);

                    canvas.drawText(strings[1],
                            ((float) x + (float) controlWidth / 2f - 10),
                            ((float) (y + move) + unitHeight / 2 + (float) textRect.height() / 2f * 2 - 5), textPaint);
                } else {
                    // 绘制内容
